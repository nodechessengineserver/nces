class wBoard extends WasmLoader.WasmLoader implements Loadable{    
    ...
    legalFens():any[]{        
        let sans=this.sortedLegalSanList().split("\n")
        if(sans[0]=="") return []        
        let fens=sans.map(san=>{            
            this.makeSanMove(san)            
            let result={san:san,fen:this.reportFen()}
            this.back()
            return result
        })
        return fens
    ...
    latestpgn:string=null
    flipByHandle(){
        let appconfig=Globals.gui.appconfig
        let handle=appconfig.handle
        let white=this.getPgnHeader("White")
        let black=this.getPgnHeader("Black")
        if(handle!=undefined){
            if(Misc.roughlyEqualStrings(handle,white)) this.flip=0
            if(Misc.roughlyEqualStrings(handle,black)) this.flip=2
        }        
    }
    setUpGame(){
    ...
    algebToMove(algeb:string):Vectors.Move{
        ...
        let promkind="-"
        if(letterparts.length>2) promkind=letterparts[2]
